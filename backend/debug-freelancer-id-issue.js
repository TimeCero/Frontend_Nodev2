...
const debugFreelancerIdIssue = async () => {
  ...
  try {
    ...
    if (testId) {
      ...
      const response = await fetch(`http://localhost:3001/api/freelancers/${testId}/profile`);
      console.log('📊 Status:', response.status);
      
      const responseText = await response.text();
      console.log('📋 Response:', responseText);
      
      // También probar con user_id si es diferente
      if (profiles[0]?.user_id && profiles[0]?.user_id !== profiles[0]?.id) {
        console.log(`\n📤 Probando con user_id: ${profiles[0].user_id}`);
        const response2 = await fetch(`http://localhost:3001/api/freelancers/${profiles[0].user_id}/profile`);
    ...
    // Verificar qué campo se debe usar para mensajes
